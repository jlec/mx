                        ...
                        int      column)
{
  ...
  clutter_actor_iter_init (&iter, (ClutterActor *) table);
  while (clutter_actor_iter_next (&iter, &child))
    {
      MxTableChild *meta_child;

      meta_child =
        (MxTableChild *) clutter_container_get_child_meta (CLUTTER_CONTAINER (table),
                                                           child);

      if ((row >= meta_child->row && row <= meta_child->row + (meta_child->row_span - 1)) &&
          (column >= meta_child->col && column <= meta_child->col + (meta_child->col_span - 1)))
        return child;
    }
                     ...
                     MxFocusable      *from)
{
  ...
  clutter_actor_iter_init (&iter, (ClutterActor *) focusable);
  while (clutter_actor_iter_next (&iter, &child_actor))
    {
      if (child_actor == (ClutterActor *) from)
        break;
      child_actor = NULL;
    }
  ...
  switch (direction)
    ...
    case MX_FOCUS_DIRECTION_NEXT:
      while (clutter_actor_iter_next (&iter, &child_actor))
        {
          if (MX_IS_FOCUSABLE (child_actor))
            {
              focused = mx_focusable_accept_focus (MX_FOCUSABLE (child_actor),
                                                   MX_FOCUS_HINT_FIRST);
    ...
    case MX_FOCUS_DIRECTION_PREVIOUS:
      while (clutter_actor_iter_prev (&iter, &child_actor))
        {
          if (MX_IS_FOCUSABLE (child_actor))
            {
              focused = mx_focusable_accept_focus (MX_FOCUSABLE (child_actor),
...
mx_table_accept_focus (MxFocusable *focusable, MxFocusHint hint)
{
  ...
  switch (hint)
    ...
    case MX_FOCUS_HINT_PRIOR:
      while (clutter_actor_iter_next (&iter, &child))
        {
          if (priv->last_focus == (MxFocusable *) child)
            break;
          child = NULL;
        }
                        ...
                        ClutterActor     *actor)
{
  ...
  rows = 0;
  cols = 0;
  clutter_actor_iter_init (&iter, (ClutterActor *) container);
  while (clutter_actor_iter_next (&iter, &child))
    {
      MxTableChild *meta =
        (MxTableChild *) clutter_container_get_child_meta (container,
                                                           child);
  ...
  while (clutter_actor_iter_next (&iter, &child))
    {
      ...
      meta =
        (MxTableChild *) clutter_container_get_child_meta (CLUTTER_CONTAINER (self),
                                                           child);
